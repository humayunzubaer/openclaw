...
/// পরের shot। Document মোডে ১০০ পাতা পর্যন্ত, ID Card মোডে ঠিক ২টা (সামনে+পেছনে)।
...
class _ScanScreenState extends State<ScanScreen> {
  ...
  DocumentKind _mode = DocumentKind.document;
  final List<ScanPage> _pages = [];
  bool _busy = false;

  Future<void> _captureLoop() async {
    if (_mode == DocumentKind.idCard) {
      await _captureOne(CaptureKind.idCardFront, label: 'আইডি কার্ড — সামনের পাশ');
      if (!mounted) return;
      await _captureOne(CaptureKind.idCardBack, label: 'আইডি কার্ড — পেছনের পাশ');
    ...
    await _captureOne(CaptureKind.document, label: 'পেজ ${_pages.length + 1}');
  }

  Future<void> _captureOne(CaptureKind kind, {required String label}) async {
    setState(() => _busy = true);
    ...
    if (result == null) {
      await _captureOne(kind, label: label); // retake — একই ধাপ আবার
      return;
    }
    final finalPath = result == 0 ? rawPath : await bakeRotation(rawPath, result);
    setState(() => _pages.add(ScanPage(id: const Uuid().v4(), imagePath: finalPath, kind: kind)));
  }
  ...
  Future<void> _finish() async {
    ...
    final doc = ScanDocument(
      id: const Uuid().v4(),
      title: _mode == DocumentKind.idCard
          ? 'আইডি কার্ড ${DateTime.now().toString().substring(0, 16)}'
          : 'স্ক্যান ${DateTime.now().toString().substring(0, 16)}',
      pages: _pages,
        ...
        children: [
          if (_pages.isEmpty)
            Padding(
              ...
              child: SegmentedButton<DocumentKind>(
                segments: const [
                  ButtonSegment(value: DocumentKind.document, label: Text('ডকুমেন্ট')),
                  ButtonSegment(value: DocumentKind.idCard, label: Text('আইডি কার্ড')),
                ],
                selected: {_mode},
          ...
          Padding(
            ...
            child: Row(
              children: [
                ...
                if (_mode == DocumentKind.document && _pages.isNotEmpty) ...[
